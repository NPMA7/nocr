...
export default function WhatsAppGateway() {
  ...
  const performAction = async (actionStr) => {
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/whatsapp/action`, {
        action: actionStr,
      });
      if (res.data.success) {
    ...
    <div className="bg-slate-800 border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
      <div className="p-5 border-b border-slate-700/50">
        ...
        <p className="text-xs text-slate-400 mt-1">
          Hubungkan satu nomor WhatsApp untuk digunakan sebagai Omnichannel
          (dibalas banyak admin) dan Auto-reply Bot.
        </p>
      </div>
            ...
            {status === "qr" && qrCode && (
              <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl w-max mb-4 mx-auto">
                <img
                  ...
                  className="w-48 h-48"
                />
                <p className="text-xs text-slate-600 font-bold mt-2 text-center">
                  Scan via WhatsApp &gt; Tautkan Perangkat
                </p>
              </div>
            )}
              ...
              {status === "connected" && (
                <button
                  onClick={() => {
                    if (
                      confirm(
                        "Yakin ingin memutus tautan nomor ini (Logout)? Anda harus scan QR ulang nanti.",
                      )
                    ) {
                  ...
                  disabled={loading}
                  className="cursor-pointer flex items-center gap-2 bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg text-xs font-semibold transition disabled:opacity-50"
                >
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500">
                  Merespon perintah seperti /ping, /info secara otomatis
                </span>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer group mt-2">
              ...
              <div className="flex flex-col">
                <span className="text-xs font-semibold text-slate-200 group-hover:text-white">
                  Pesan Otomatis Luar Jam Kerja
                </span>
                <span className="text-xs text-slate-500">
                  Mengirim balasan default saat ada pesan baru
                </span>
              </div>
            </label>
