...
export default function RoleSettings({ showToast }) {
    ...
    const openEdit = (r) => {
        ...
        let perms = {};
        try {
            let parsed = typeof r.permissions === 'string' ? JSON.parse(r.permissions) : r.permissions;
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="text-sm text-slate-300">
                        {loading ? (
                            ...
                            const configuredMenus = Object.keys(perms).filter(k => perms[k] && perms[k].length > 0).length;
                ...
                <div className="fixed inset-0 z-[4000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl max-w-2xl w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-400 mb-3 border-b border-slate-700 pb-2">Konfigurasi Akses Menu</label>
                                <div className="border border-slate-700 rounded-lg overflow-hidden">
                                    <table className="w-full text-sm text-left">
                                        <thead className="bg-slate-700/50 text-slate-300">
                                            <tr>
                                                <th className="px-4 py-2.5 font-medium">Menu</th>
                                                <th className="px-4 py-2.5 font-medium text-center">Aksi Akses (CRUD)</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-700/50 bg-slate-800">
                                            {Object.entries(MENUS).map(([menuKey, menuLabel]) => {
                                                ...
                                                return (
                                                    <tr key={menuKey} className="hover:bg-slate-700/20 transition-colors">
                                                        <td className="px-4 py-3 text-slate-200 font-medium">{menuLabel}</td>
                                                                    ...
                                                                    <label key={action} className="flex items-center gap-1.5 cursor-pointer group">
                                                                        <div className="relative flex items-center">
                                                                            <input
                                                                                ...
                                                                                onChange={() => togglePerm(menuKey, action)}
                                                                            />
                                                                        ...
                                                                        <span className="text-xs text-slate-400 group-hover:text-slate-200 capitalize select-none">{action}</span>
                                                                    </label>
                                                                ))}
                                                            </div>
                                                        </td>
