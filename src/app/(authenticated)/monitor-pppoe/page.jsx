...
export default function MonitorPppoe() {
  ...
  const handleSaveMapping = async () => {
    ...
    try {
      ...
      if (existing) {
        setMappings(mappings.map(m => m.ruijie_mac === res.data.ruijie_mac ? { ...m, ...res.data } : m));
      } else {
    ...
    <div className="h-full min-h-0 flex flex-col gap-4 overflow-hidden relative">
      ...
      <div className="flex-shrink-0 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Status Access Point (Ruijie) & Mikrotik (PPPoE)</p>
        </div>
        <div className="flex items-center gap-4">
            <button 
              onClick={() => fetchData()}
              disabled={loading}
              className="cursor-pointer flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition shadow-lg bg-purple-600 hover:bg-purple-700 border border-purple-500 text-white shadow-purple-500/20"
        ...
        <div className="p-4 border-b border-slate-700/30 flex items-center gap-3 flex-shrink-0 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-3 py-1.5 text-sm text-slate-100 focus:border-purple-500 outline-none w-full"
            />
          </div>
          ...
          <select
            ...
            className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1.5 text-sm text-slate-300 outline-none focus:border-purple-500 cursor-pointer"
          >
                    ...
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] bg-purple-500/20 text-purple-400 px-1.5 py-0.5 rounded border border-purple-500/20">PPPoE</span>
                        </div>
                        ...
                        {editingPrefixMac === d.ruijie_mac ? (
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="cursor-pointer bg-slate-900 border border-slate-700 rounded px-2 py-1 text-sm text-slate-100 outline-none focus:border-purple-500 w-full min-w-[150px]"
                              autoFocus
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-800 border border-slate-700 shadow-2xl rounded-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 flex flex-col gap-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 block">Ruijie Access Point (PPPoE)</label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 block">Pilih Akun Mikrotik (PPPoE)</label>
                <select 
                  value={selectedMikrotikName}
                  onChange={e => setSelectedMikrotikName(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2.5 text-sm text-slate-100 focus:border-purple-500 outline-none"
                >
                ...
                <p className="text-[10px] text-slate-500 mt-1.5 leading-relaxed">
                  Pilih nama secret PPPoE yang benar dari Mikrotik. Pilihan ini akan menimpa pencocokan nama otomatis.
                </p>
              </div>
            </div>
            <div className="p-4 border-t border-slate-700 bg-slate-800/80 flex items-center justify-end gap-3">
              ...
              <button 
                ...
                className="cursor-pointer px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
