...
function MediaMessage({ msgId }) {
  ...
  if (media) {
    if (media.mimetype.startsWith("image/")) {
      return (
        <img
          src={`data:${media.mimetype};base64,${media.media}`}
          alt="Media"
          className="w-full max-w-[400px] rounded-md mt-1 mb-1 max-h-[400px] object-contain bg-slate-900/50"
        />
      );
      ...
      return (
        <video
          controls
          src={`data:${media.mimetype};base64,${media.media}`}
          className="w-full max-w-[400px] rounded-md mt-1 mb-1 max-h-[400px] bg-slate-900/50"
        />
      );
      ...
      return (
        <audio
          controls
          src={`data:${media.mimetype};base64,${media.media}`}
          className="w-full mt-1 mb-1 max-w-[200px]"
        />
      );
    }
    return (
      <a
        href={`data:${media.mimetype};base64,${media.media}`}
        download={media.filename || "berkas"}
        className="inline-flex items-center gap-1 text-blue-300 hover:text-blue-200 underline text-xs mt-1 mb-1 p-2 bg-slate-800/50 rounded"
      >
    ...
    <div className="text-xs mb-1 border border-slate-500/30 p-2 rounded bg-slate-800/50 flex flex-col gap-2">
      <div className="flex items-center justify-between gap-3">
        <span className="italic flex items-center gap-1 text-slate-300">
          <ImageIcon size={14} />{" "}
          {loading ? "Memuat media..." : "Terdapat media"}
        </span>
        {loading && (
...
export default function LiveChatPage() {
  ...
  const handleNewMessage = (msg) => {
    ...
    setChats((prev) => {
      const chatIndex = prev.findIndex(
        (c) => c.id === msg.from || c.id === msg.to,
      );
      if (chatIndex > -1) {
        const newChats = [...prev];
        newChats[chatIndex].lastMessage = {
          body: msg.body,
        ...
        if (!msg.fromMe && (!selectedChat || selectedChat.id !== msg.from)) {
          newChats[chatIndex].unreadCount =
  ...
  const loadChat = async (chat) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/whatsapp/chat/${encodeURIComponent(chat.id)}`,
      );
  ...
  const getAckIcon = (ack) => {
    ...
    if (ack === 3 || ack === 4)
      return <CheckCheck size={12} className="text-blue-400" />; // read
      ...
      <div className="flex-none p-4 border-b border-slate-700/50 bg-slate-800 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Satu nomor WhatsApp, dikelola bersama
          </p>
        </div>
      </div>
        ...
        <div className="w-1/3 md:w-80 flex flex-col border-r border-slate-700/50 bg-slate-800/50 min-h-0">
          <div className="p-3 border-b border-slate-700/50">
            <input
              ...
              placeholder="Cari..."
              className="w-full bg-slate-900 border border-slate-700 p-2 text-xs text-slate-200 rounded-lg outline-none focus:border-blue-500"
            />
          </div>
          ...
          {selectedChat ? (
            <>
                ...
                {loadingMessages ? (
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      <div
                        key={msg.id}
                        className={`flex flex-col max-w-[80%] min-w-0 ${isMe ? "self-end items-end" : "self-start items-start"}`}
                      >
                        <div
                          className={`p-2.5 rounded-xl text-xs shadow-md overflow-hidden ${isMe ? "bg-emerald-600 text-white rounded-tr-sm" : "bg-slate-700 text-slate-100 rounded-tl-sm"}`}
                        >
                        ...
                        <div
                          className={`text-[10px] ${isMe ? "text-slate-400" : "text-slate-500"} mt-1 flex items-center gap-1`}
                        >
                          {formatTime(msg.timestamp)}{" "}
                          {isMe && getAckIcon(msg.ack)}
                        </div>
                      </div>
                    );
                  })
              ...
              {hasAccess(sessionUser, "chat", "create") ? (
                <div className="p-3 bg-slate-800 border-t border-slate-700/50">
                  <form onSubmit={sendMessage} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-900 border border-slate-700 p-3 text-xs text-slate-100 rounded-xl outline-none focus:border-emerald-500 transition"
                    />
                    <button
                      type="submit"
                ...
                <div className="p-4 bg-slate-800 border-t border-slate-700/50 flex items-center justify-center text-slate-500 text-xs gap-2">
                  <ShieldAlert size={16} /> Anda tidak memiliki hak akses untuk
                  membalas pesan.
                </div>
              )}
            </>
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-slate-500 p-10 text-center">
              ...
              <p className="text-xs">
                Pilih kontak di sebelah kiri untuk mulai membalas pesan.
              </p>
            </div>
          )}
        </div>
