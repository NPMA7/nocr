...
function StatusBadge({ online }) {
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${online ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
...
function IfaceBadge({ running, disabled }) {
  if (disabled === 'true') return <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-600/60 text-slate-400">Disabled</span>;
  if (running === 'true') return <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400">Up</span>;
  return <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-400">Down</span>;
}
...
function TopologyContent() {
  ...
  const [manualAddData, setManualAddData] = useState({ label: '', type: 'client', lat: '', lng: '', addressSearch: '', linked_interface: '', vendor: '' });
  ...
  const trackNodeChanges = (prevNodes, nextNodes) => {
    ...
    for (const n of nextNodes) {
      if (!prevIds.has(n.id) || isNodeDirty(n, baselineNodesRef.current[n.id])) {
  ...
  const trackEdgeChanges = (prevEdges, nextEdges) => {
    ...
    for (const e of nextEdges) {
      if (!prevIds.has(e.id) || isEdgeDirty(e, baselineEdgesRef.current[e.id])) {
  ...
  const applyTopologyFromServer = (serverNodes, serverEdges, { resetBaseline = true, toastMsg = null } = {}) => {
  ...
  const mergeTopologyFromRemote = (remoteNodes, remoteEdges, remoteRevision) => {
  ...
  const handleMetricsUpdate = (data) => {
    setNodes(prev => prev.map(n => n.device_id === data.id ? { ...n, status: data.status } : n));
  ...
  useEffect(() => {
    if (focusId && nodes.length > 0) {
      ...
      if (targetNode && !isNaN(targetNode.latitude) && !isNaN(targetNode.longitude)) {
        setFlyToTarget({ lat: targetNode.latitude, lng: targetNode.longitude, zoom: 17 });
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      if (selectedNode.device_id) {
        axios.get(`${API_URL}/monitor/${selectedNode.device_id}`).then(res => {
          setNodeDetail({ ...res.data, loading: false });
        }).catch(() => {
          setNodeDetail({ error: 'Koneksi API Gagal', loading: false });
        });
        axios.get(`${API_URL}/devices/${selectedNode.device_id}`).then(res => {
  ...
  const handleAddNode = (lat, lng, type, label = null, linkedInterface = null, vendor = null) => {
    if (readOnly) return;
    const newNode = {
      id: 'node_' + Date.now(),
    ...
    if (interactionMode === 'add_edge') {
      if (!linkStartNode) {
        ...
        if (linkStartNode !== node.id) {
          const newEdge = { id: 'edge_' + Date.now(), from_node: linkStartNode, to_node: node.id, label: 'Kabel FO', status: 'up' };
  ...
  const saveLayout = async () => {
    ...
    try {
      ...
      console.error(e);
      addToast('Gagal menyimpan peta: ' + (e.response?.data?.error || e.message), 'error');
    }
  };
  ...
  const ifaceGroups = useMemo(() => {
    const groups = {};
    const filtered = coreInterfaces.filter(i => 
      !ifacePanelSearch || 
      (i.name && i.name.toLowerCase().includes(ifacePanelSearch.toLowerCase()))
    );
    filtered.forEach(i => {
  ...
  const runningCount = coreInterfaces.filter(i => i.running === 'true' && i.disabled !== 'true').length;
  const downCount = coreInterfaces.filter(i => i.running !== 'true' && i.disabled !== 'true').length;

  const mapNodes = useMemo(() => {
    if (nodeViewFilter === 'client') return nodes.filter(isClientNode);
    if (nodeViewFilter === 'infrastructure') return nodes.filter(isInfrastructureNode);
    return nodes;
  ...
  return (
    <div className="flex flex-col h-full min-h-0 -m-4 md:-m-6 relative overflow-hidden bg-slate-950">
      ...
      {/* Map Control Toolbar */}
      <div className="flex-shrink-0 bg-slate-800 border-b border-slate-700/50 px-3 py-2 md:px-6 md:py-3 flex flex-col md:flex-row justify-between items-start md:items-center z-[1000] gap-3 md:gap-4 overflow-hidden">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3 w-full md:w-auto">
          ...
          <div className="w-full md:w-auto pb-1 md:pb-0 flex-shrink-0 min-md:flex">
            {readOnly && isVisitorRole(getStoredUser().role) ? (
              <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-1.5">
                ...
                <span className="text-xs font-semibold text-amber-300">Readonly (Visitor)</span>
              </div>
            ) : canEdit ? (
            <div className="flex flex-wrap bg-slate-900 rounded-lg p-1 border border-slate-700">
              <button
                onClick={() => { setInteractionMode('select'); setLinkStartNode(null); }}
              ...
              <button
                onClick={() => { setInteractionMode('add_node'); setLinkStartNode(null); }}
              ...
              <button
                onClick={() => { setShowManualAddModal(true); setManualIfaceSearch(''); setShowManualIfaceDropdown(false); }}
              ...
              <button
                onClick={() => { setInteractionMode('add_edge'); setLinkStartNode(null); }}
              ...
              <button
                onClick={() => { setInteractionMode('delete_edge'); setLinkStartNode(null); }}
            ...
            <div className="cursor-pointer flex flex-wrap bg-slate-900 rounded-lg p-1 border border-slate-700">
              ...
              <div className="w-px bg-slate-700/50 mx-1 hidden sm:block"></div>
              <button
                onClick={() => setFlyToTarget({ lat: -7.0225, lng: 107.527, zoom: 16.5 })}
              ...
              <button
                onClick={() => setFlyToTarget({ lat: -7.065, lng: 107.55, zoom: 11 })}
                className="cursor-pointer flex-1 min-w-fit px-3 py-1.5 rounded-md text-xs font-semibold flex items-center justify-center gap-1.5 transition text-slate-400 hover:text-white hover:bg-slate-800"
              >
          ...
          {!readOnly && interactionMode === 'add_node' && (
            <div className="w-full md:w-auto overflow-x-auto hide-scrollbar pb-1 md:pb-0 flex-shrink-0">
              <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-700 rounded-lg p-1 w-max">
                <span className="text-[10px] text-slate-400 uppercase font-bold px-2 whitespace-nowrap">TIPE:</span>
                {['olt', 'odc', 'odp', 'pole', 'client'].map(t => (
                  <button key={t} onClick={() => setNewNodeType(t)} className={`cursor-pointer px-2.5 py-1 rounded text-xs uppercase whitespace-nowrap ${newNodeType === t ? 'bg-slate-700 text-white font-bold' : 'text-slate-400 hover:text-white'}`}>{t.toUpperCase()}</button>
                ))}
              </div>
            </div>
          )}
      ...
      <div className="flex-1 min-h-0 flex relative">
        ...
        <div className="absolute top-4 right-4 z-[9999] flex flex-col items-end gap-2 pointer-events-none">
          {toasts.map(t => (
            <div key={t.id} className={`px-4 py-3 max-w-md break-words rounded-xl shadow-2xl backdrop-blur-md font-medium text-sm animate-fade-in ${t.type === 'error' ? 'bg-red-500/90 text-white border border-red-400/50' : 'bg-emerald-500/90 text-white border border-emerald-400/50'}`}>
              {t.msg}
            </div>
          ))}
        </div>
          ...
          <div className="absolute inset-0 z-[3000] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
            <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md max-h-[min(90dvh,100%)] my-auto flex flex-col overflow-hidden animate-fade-in-up">
              <div className="flex-shrink-0 p-4 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/50">
                <h3 className="font-bold text-slate-100 flex items-center gap-2"><MapPin size={18} className="text-emerald-400" /> Tambah Titik Node Manual</h3>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5 relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setManualAddData(prev => ({...prev, label: e.target.value, linked_interface: ''}));
                  ...
                  {showManualIfaceDropdown && combinedInterfaceOptions.length > 0 && (
                    <div className="absolute top-[64px] left-0 right-0 z-[4000] bg-slate-800 border border-slate-700 rounded-lg shadow-2xl max-h-52 overflow-auto">
                      <div
                        ...
                        onClick={() => {
                          setManualAddData(prev => ({...prev, label: '', linked_interface: ''}));
                      ...
                      {combinedInterfaceOptions
                        ...
                        .map((iface, idx) => {
                          ...
                          <div
                            ...
                            onClick={() => {
                              if (isUsed) return;
                              setManualAddData(prev => ({...prev, label: iface.name, linked_interface: iface.name}));
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-400">Tipe Node</label>
                  <select
                    value={manualAddData.type}
                    onChange={(e) => setManualAddData({...manualAddData, type: e.target.value})}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-400">Vendor / Merek ISP (Opsional)</label>
                  <input
                    type="text"
                    placeholder="Contoh: Indibiz, Megavision, etc.."
                    value={manualAddData.vendor}
                    onChange={(e) => setManualAddData({...manualAddData, vendor: e.target.value})}
                    className="bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 w-full"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => setManualAddData({...manualAddData, lat: e.target.value})}
                      className="bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 w-full"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => setManualAddData({...manualAddData, lng: e.target.value})}
                      className="bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 w-full"
                    />
                  </div>
                </div>
              ...
              <div className="flex-shrink-0 p-4 border-t border-slate-700/50 bg-slate-800/50 flex justify-end gap-2">
                <button onClick={() => { setShowManualAddModal(false); setManualIfaceSearch(''); }} className="px-4 py-2 rounded-lg text-sm font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition">Batal</button>
                <button
                  onClick={() => {
                    ...
                    handleAddNode(manualAddData.lat, manualAddData.lng, manualAddData.type, manualAddData.label || manualIfaceSearch || 'Titik Manual', manualAddData.linked_interface || null, manualAddData.vendor || null);
                    setFlyToTarget({ lat: parseFloat(manualAddData.lat), lng: parseFloat(manualAddData.lng) });
                    setShowManualAddModal(false);
                    setManualAddData({ label: '', type: 'client', lat: '', lng: '', addressSearch: '', linked_interface: '', vendor: '' });
                    setManualIfaceSearch('');
          ...
          {coreStatus && (
            <div className={`rounded-xl border p-3.5 shadow-xl backdrop-blur-sm pointer-events-auto ${coreStatus.connected ? 'bg-slate-900/95 border-emerald-500/30' : 'bg-slate-900/95 border-red-500/30'}`}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${coreStatus.connected ? 'bg-emerald-400 animate-pulse' : 'bg-red-400'}`} />
                  <span className="text-xs font-bold text-slate-200">{coreStatus.device_name || 'MikroTik Pusat'}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  <div className="bg-slate-800/80 rounded-lg p-2 flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs font-bold text-slate-200">{coreStatus.cpu}%</p>
                    </div>
                  </div>
                  <div className="bg-slate-800/80 rounded-lg p-2 flex items-center gap-2 min-w-0">
                    <Network size={13} className="text-emerald-400 flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] text-slate-500">Active Pelanggan</p>
                      <p className="text-xs font-bold text-emerald-400">{coreStatus.l2tp_active+coreStatus.pppoe_active}</p>
                    </div>
                  </div>
                  <div className="bg-slate-800/80 rounded-lg p-2 flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs font-bold text-slate-200 truncate">{coreStatus.uptime}</p>
                    </div>
                  </div>
                  <div className="bg-slate-800/80 rounded-lg p-2 flex items-center gap-2 min-w-0">
                    <RefreshCw size={13} className="text-purple-400 flex-shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] text-slate-500">Board</p>
                      <p className="text-xs font-bold text-slate-200 truncate">{coreStatus.board}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-red-400 mt-1">{coreStatus.error}</p>
              )}
            </div>
          )}
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/95 shadow-xl backdrop-blur-sm pointer-events-auto">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-200">Interfaces ({coreInterfaces.length})</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-bold">{runningCount} UP</span>
                  {downCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-400 font-bold">{downCount} DOWN</span>}
                </div>
                {showIfacePanel ? <ChevronUp size={14} className="cursor-pointer text-slate-400" /> : <ChevronDown size={14} className="cursor-pointer text-slate-400" />}
              </button>
              {showIfacePanel && (
                <div className="border-t border-slate-700/50 max-h-64 flex flex-col">
                  ...
                  <div className="overflow-auto flex-1">
                    {Object.entries(ifaceGroups).length === 0 ? (
                      <div className="p-4 text-center text-xs text-slate-500">Tidak ada interface</div>
                    ) : Object.entries(ifaceGroups).map(([type, ifaces]) => (
                    <div key={type}>
                      <div className="px-3 py-1.5 bg-slate-800/50 text-[10px] font-bold text-slate-500 uppercase tracking-wider">{type}</div>
                      {ifaces.map((iface, i) => (
                        <div key={i} className="px-3 py-2 flex items-center justify-between border-b border-slate-800/80 hover:bg-slate-800/40 transition">
                          <div className="flex items-center gap-2">
                            <div className={`w-1.5 h-1.5 rounded-full ${iface.running === 'true' && iface.disabled !== 'true' ? 'bg-emerald-400' : iface.disabled === 'true' ? 'bg-slate-500' : 'bg-red-400'}`} />
                            <span className="text-xs text-slate-200 font-medium">{iface.name}</span>
                          </div>
                          <IfaceBadge running={iface.running} disabled={iface.disabled} />
                        </div>
                      ))}
        ...
        <div className="hidden md:flex absolute top-3 right-3 z-[1000] flex-col gap-2 pointer-events-none">
          ...
          <div className="rounded-xl border border-slate-700/50 bg-slate-900/95 shadow-xl backdrop-blur-sm pointer-events-auto p-3">
            ...
            <div className="flex flex-col gap-1.5 text-[10px] text-slate-400">
              ...
              <span className="flex items-center gap-2"><span className="w-6 h-1 rounded inline-block" style={{backgroundImage:'repeating-linear-gradient(90deg,#ef4444 0,#ef4444 4px,transparent 4px,transparent 10px)'}} /> DOWN</span>
              <span className="flex items-center gap-2"><span className="w-6 h-1 rounded inline-block" style={{backgroundImage:'repeating-linear-gradient(90deg,#475569 0,#475569 3px,transparent 3px,transparent 7px)'}} /> Disabled</span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-700/50 bg-slate-900/95 shadow-xl backdrop-blur-sm pointer-events-auto p-3 min-w-[200px]">
            ...
            <div className="flex flex-col gap-1.5">
              <button
                type="button"
                onClick={() => setMapTheme(mapTheme === 'dark' ? 'colored' : 'dark')}
              ...
              >
                <span className={`fa ${mapTheme === 'dark' ? 'fa-moon' : 'fa-sun'} text-[10px]`} />
                Tema Peta
              </button>
              <button
                type="button"
              ...
              >
                <span className={`fa ${showLabels ? 'fa-eye' : 'fa-eye-slash'} text-[9px]`} />
                Label Node
              </button>
              <button
                type="button"
                onClick={() => setNodeViewFilter((f) => (f === 'client' ? 'all' : 'client'))}
                className={`cursor-pointer w-full px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition flex items-center justify-center gap-1.5
              ...
              <button
                type="button"
                onClick={() => setNodeViewFilter((f) => (f === 'infrastructure' ? 'all' : 'infrastructure'))}
                className={`cursor-pointer w-full px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition flex items-center justify-center gap-1.5
        ...
        {/* Node Sidebar */}
        <div className={`absolute top-0 right-0 bottom-0 w-80 bg-slate-800/95 backdrop-blur-md border-l border-slate-700/50 flex flex-col z-[1000] shadow-2xl transition-transform duration-300 ease-out ${currentSelectedNode ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-4 border-b border-slate-700/50 flex justify-between items-center">
            <div className="flex items-center gap-3">
              <h3 className="font-bold text-slate-100">Properties Node</h3>
              {currentSelectedNode && (
                <button
                  onClick={() => setFlyToTarget({ lat: currentSelectedNode.latitude, lng: currentSelectedNode.longitude, zoom: 17 })}
                ...
                >
                  <MapPin size={14} /> <span className="font-bold text-xs text-slate-100 ml-1 cursor-pointer">Zoom Location</span>
                </button>
              )}
            </div>
            <button className="text-slate-400 hover:text-white" onClick={() => setSelectedNode(null)}><X size={20} /></button>
          </div>
            ...
            {currentSelectedNode && (
              <>
                {activeNodeTab === 'identitas' ? (
                  <>
                    ...
                    {combinedInterfaceOptions.length > 0 && (
                      <div className="flex flex-col gap-1.5 relative">
                        ...
                        <input
                          ...
                          onBlur={() => setTimeout(() => setShowNodeIfaceDropdown(false), 200)}
                          className={`bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500 w-full ${readOnly ? 'opacity-70 cursor-default' : ''}`}
                        />
                        ...
                        {!readOnly && showNodeIfaceDropdown && (
                          <div className="absolute top-[64px] left-0 right-0 z-[2000] bg-slate-800 border border-slate-700 rounded-lg shadow-2xl max-h-52 overflow-auto">
                            <div
                              className="px-3 py-2.5 text-xs text-slate-400 hover:bg-slate-700 cursor-pointer"
                              onClick={() => {
                                setNodesFromUser(prev => prev.map(n => n.id === currentSelectedNode.id ? { ...n, linked_interface: '' } : n));
                                setNodeIfaceSearch('');
                                setShowNodeIfaceDropdown(false);
                              }}
                            >
                              ...
                              .map((iface, i) => {
                                ...
                                <div
                                  ...
                                  onClick={() => {
                                    ...
                                    setNodesFromUser(prev => prev.map(n => n.id === currentSelectedNode.id ? { ...n, linked_interface: iface.name, label: iface.name } : n));
                                ...
                                >
                                  <span className="font-medium flex items-center gap-1.5">
                                    {iface.name}
                                    {isUsedByOther && <span className="text-[9px] bg-slate-700 text-slate-400 px-1.5 py-0.5 rounded">Terpakai</span>}
                                  </span>
                                  <span className="text-[10px] text-slate-500 uppercase">{iface.type}</span>
                                </div>
                            )})}
                    ...
                    {currentSelectedNode.device_id && nodeDetail && !nodeDetail.loading && !nodeDetail.error && (
                      <div className="flex flex-col gap-3 mt-1 bg-slate-900/40 p-3 rounded-lg border border-slate-700/40">
                        <p className="text-[10px] text-slate-500 uppercase font-bold">Live Metrics</p>
                        <div className="flex justify-between items-center text-xs"><span className="text-slate-400">Uptime</span><span className="text-slate-200">{nodeDetail?.uptime || '-'}</span></div>
                        <div className="flex justify-between items-center text-xs"><span className="text-slate-400">PPPoE Aktif</span><span className="text-emerald-400 font-semibold">{nodeDetail?.pppoe_active || 0} user</span></div>
                        <div className="flex justify-between items-center text-xs"><span className="text-slate-400">CPU</span><span className="text-slate-200">{nodeDetail?.cpu || 0}%</span></div>
                      </div>
                    )}
                  ...
                  <>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400">Lat, Long (Lintang, Bujur)</label>
                      <input
                        type="text"
                        readOnly={readOnly}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400">Nama PIC</label>
                      <input
                        type="text"
                        readOnly={readOnly}
                        placeholder="Nama penanggung jawab titik"
                        value={currentSelectedNode.pic_name || ''}
                        onChange={(e) => setNodesFromUser(prev => prev.map(n => n.id === currentSelectedNode.id ? { ...n, pic_name: e.target.value } : n))}
                        className={`bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500 w-full ${readOnly ? 'opacity-70 cursor-default' : ''}`}
                      />
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-slate-400">Nomor PIC</label>
                      <input
                        type="text"
                        readOnly={readOnly}
                        placeholder="Contoh: 08123456789"
                        value={currentSelectedNode.pic_phone || ''}
                        onChange={(e) => setNodesFromUser(prev => prev.map(n => n.id === currentSelectedNode.id ? { ...n, pic_phone: e.target.value } : n))}
                        className={`bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500 w-full ${readOnly ? 'opacity-70 cursor-default' : ''}`}
                      />
                    </div>
                  </>
                ...
                <button onClick={() => {
                  ...
                  setNodesFromUser(prev => prev.filter(n => n.id !== selectedNode.id));
                  setEdgesFromUser(prev => prev.filter(e => e.from_node !== selectedNode.id && e.to_node !== selectedNode.id && e.from !== selectedNode.id && e.to !== selectedNode.id));
                  setSelectedNode(null);
        ...
        {/* Edge Sidebar */}
        <div className={`absolute top-0 right-0 bottom-0 w-80 bg-slate-800/95 backdrop-blur-md border-l border-slate-700/50 flex flex-col z-[1000] shadow-2xl transition-transform duration-300 ease-out ${selectedEdge ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-4 border-b border-slate-700/50 flex justify-between items-center">
            <h3 className="font-bold text-slate-100">Koneksi FO</h3>
            <button className="text-slate-400 hover:text-white" onClick={() => setSelectedEdge(null)}><X size={20} /></button>
          </div>
          <div className="p-5 flex-1 flex flex-col gap-4">
            {selectedEdge && (
              <>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-400">Nama Kabel / Interface</label>
                  <input
                    type="text"
                    readOnly={readOnly}
                    value={selectedEdge.label || ''}
                    onChange={(e) => setEdgesFromUser(prev => prev.map(ed => ed.id === selectedEdge.id ? { ...ed, label: e.target.value } : ed))}
                    className={`bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-sm text-slate-100 focus:outline-none focus:border-blue-500 w-full ${readOnly ? 'opacity-70 cursor-default' : ''}`}
                  />
                  <p className="text-[10px] text-slate-500">Jika nama cocok dengan interface MikroTik, warna kabel akan mengikuti status interface secara otomatis.</p>
                </div>

                {/* Show matched interface status */}
                {selectedEdge.label && (() => {
                  const matched = coreInterfaces.find(i => i.name && selectedEdge.label && i.name.toLowerCase() === selectedEdge.label.toLowerCase());
                  if (!matched) return (
                    <div className="bg-slate-900/40 rounded-lg p-3 border border-slate-700/40 text-xs text-slate-500">
                      Tidak ada interface MikroTik yang cocok dengan nama <strong className="text-slate-400">"{selectedEdge.label}"</strong>
                    </div>
                  );
                  return (
                    <div className={`flex items-center justify-between p-3 rounded-lg border text-xs ${matched.running === 'true' ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                      <div>
                        <p className="text-slate-400">Interface: <span className="text-slate-200 font-medium">{matched.name}</span></p>
                        <p className="text-slate-500 mt-0.5">MAC: {matched['mac-address'] || '-'} · MTU: {matched.mtu || '-'}</p>
                      </div>
                      <IfaceBadge running={matched.running} disabled={matched.disabled} />
                    </div>
                  );
                })()}

                {/* Manual override status */}
                {!coreInterfaces.find(i => i.name?.toLowerCase() === selectedEdge.label?.toLowerCase()) && (
                  <div className="flex justify-between items-center py-2.5 border-b border-slate-700/30">
                    <span className="text-xs text-slate-400">Status Manual</span>
                    <select
                      disabled={readOnly}
                      value={selectedEdge.status || 'up'}
                      onChange={(e) => setEdgesFromUser(prev => prev.map(ed => ed.id === selectedEdge.id ? { ...ed, status: e.target.value } : ed))}
                ...
                {!readOnly && (
                <button onClick={() => { markEdgeDeleted(selectedEdge.id); setEdgesFromUser(prev => prev.filter(e => e.id !== selectedEdge.id)); setSelectedEdge(null); }} className="mt-auto flex items-center justify-center gap-2 bg-red-500/10 hover:bg-red-500 text-red-400 hover:text-white py-2.5 rounded-lg text-sm font-semibold transition">
...
export default function Topology() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-full min-h-[200px] bg-slate-950 text-slate-400">Memuat Peta...</div>}>
      <TopologyContent />
    </Suspense>
  );
}
