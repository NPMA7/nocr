...
function TopologyContent() {
  ...
  const handleNodeClick = (e, node, waypoints = []) => {
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex relative">
        ...
        <div className="flex-1 w-full relative z-0 flex flex-col">
          ...
          {/* Floating Bottom-Right Fullscreen Focus Button */}
          <div className="absolute bottom-6 right-6 z-[950] flex items-center gap-2 pointer-events-auto">
            <button
              onClick={toggleFullscreen}
              title={isFullscreen ? "Keluar Layar Penuh (Esc)" : "Fokus Layar Penuh (Fullscreen)"}
              className={`cursor-pointer px-3.5 py-2.5 rounded-xl shadow-2xl backdrop-blur-md transition-all duration-300 flex items-center gap-2 font-bold text-xs border ${
                isFullscreen
                  ? "bg-amber-500 text-slate-950 border-amber-400 hover:bg-amber-400 shadow-amber-500/30"
                  : "bg-slate-900/90 text-slate-100 hover:text-white border-slate-700/80 hover:bg-slate-800 hover:border-blue-500/80 shadow-black/60 hover:shadow-blue-500/20"
              }`}
            >
              {isFullscreen ? (
                <>
                  <Minimize2 size={16} className="animate-pulse text-slate-950" />
