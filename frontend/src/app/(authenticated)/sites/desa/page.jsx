...
function normalizeVendorGroup(vendorName) {
  ...
  if (upper.includes("BABBAGE")) return "BABBAGE";
...
function SitesListPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((d) => {
      ...
      // Vendor Filter (supports grouped "BABBAGE")
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Profil operasional per site Desa — prefix perangkat, vendor ISP, kontak PIC, dan koordinat GIS
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col min-w-0 bg-slate-900 border border-slate-800 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="p-3 sm:p-4 border-b border-slate-800 flex items-center gap-2.5 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Cari nama desa, vendor, PIC, no. telp, alamat..."
              value={search}
              onChange={(e) => {
        ...
        <div className="overflow-x-auto custom-scrollbar min-w-0">
          {loading && items.length === 0 ? (
            <div className="p-8 text-center text-slate-400 flex flex-col items-center justify-center gap-2">
              ...
              <span className="text-xs">Memuat data wilayah desa...</span>
            </div>
            ...
            <div className="p-12 text-center">
              <p className="text-xs text-slate-400">
                Tidak ada data desa yang cocok dengan filter pencarian.
              </p>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-950 text-slate-400 border-b border-slate-800 font-mono text-[11px] uppercase tracking-wider">
                      <th className="py-2.5 px-4 font-semibold">Nama Site / Desa</th>
                        ...
                        >
                          ...
                          <td className="py-3 px-4 max-w-[240px]">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded font-bold shrink-0 tag-desa">
                                DESA
                              </span>
                            </div>
                            {d.ruijie_mac && (
                              <div className="flex items-center gap-1.5 mt-1 text-[10px] font-mono text-slate-500">
                                ...
                                <button
                                  onClick={() => copyToClipboard(d.ruijie_mac, `mac_${d.ruijie_mac}`)}
                              ...
                              <div className="space-y-0.5">
                                ...
                                {customerId && (
                                  <div className="text-[11px] font-mono text-blue-300 flex items-center gap-1">
                                    ...
                                    <button
                                      onClick={() => copyToClipboard(customerId, `cid_${d.ruijie_mac}`)}
                                      className="cursor-pointer text-slate-500 hover:text-slate-300"
                          ...
                          <td className="py-3 px-4 text-right">
                            <button
                              type="button"
                              onClick={() => router.push(`/sites/desa/${encodeMac(d.ruijie_mac)}`)}
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-[10px] font-mono px-1.5 py-0.2 rounded font-bold shrink-0 tag-desa">
                            DESA
                          </span>
                        </div>
                        <button
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                        ...
                        <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                          ...
                          {pics.length > 0 ? (
                            ...
                            <span className="text-slate-500 text-xs block mt-0.5">—</span>
                          )}
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-2.5 border-t border-slate-800 flex items-center justify-between flex-wrap gap-2 text-xs bg-slate-950">
            <span className="text-slate-400 font-medium">
              {itemsPerPage === "all"
                ? `Menampilkan ${filtered.length} dari ${filtered.length} desa`
                : `Menampilkan ${Math.min((currentPage - 1) * itemsPerPage + 1, filtered.length)}-${Math.min(currentPage * itemsPerPage, filtered.length)} dari ${filtered.length} desa`}
            </span>
...
export default function SitesPage() {
  return (
    <Suspense
      fallback={
        <div className="p-6 text-slate-400 text-xs">Memuat data wilayah desa...</div>
      }
    >
      <SitesListPage />
    </Suspense>
