...
export default function MonitorOpd() {
  ...
  const handleOpenOntWeb = (d) => {
    if (!canUpdate) {
      showToast("Anda tidak memiliki izin (Update) untuk mengakses Web ONT", "error");
      return;
    }
  ...
  const handleSaveModal = async () => {
    ...
    try {
      ...
      showToast("Pengaturan prefix & tautan OPD berhasil disimpan", "success");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Status Sinkronisasi Access Point (Ruijie) & MikroTik Client OPD
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
              ...
              <div className="lg:hidden flex flex-col gap-2.5 p-2.5 sm:p-3 bg-slate-950/40">
                {filteredDevices.length === 0 ? (
                  ...
                  paginatedDevices.map((d, i) => (
                    ...
                    >
                      {/* Baris 1: Prefix & Tag OPD (Kiri) | Status Online/Offline (Rata Kanan) */}
